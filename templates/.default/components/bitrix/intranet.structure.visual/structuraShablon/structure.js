	...
	if (!this.employeesPopup)
	{
		...
		for (var i=0; i < this.employees.length; i++)
		{
			var obEmployee = BX.create('DIV', {
				...
				html: '<a class="ui-icon ui-icon-common-user structure-avatar" href="'+this.employees[i].PROFILE+'"><i ' + (this.employees[i].PHOTO ? ' style="background: url(\''+this.employees[i].PHOTO+'\') no-repeat scroll center center transparent; background-size: cover;"' : '') + '></i></a><div class="structure-employee-name"><a href="'+this.employees[i].PROFILE+'">' + this.employees[i].NAME + '</a></div>' + (this.employees[i].POSITION ? '<div class="s1ak structure-employee-post">'+BX.util.htmlspecialchars(this.employees[i].POSITION)+'</div>' : '')
			});
